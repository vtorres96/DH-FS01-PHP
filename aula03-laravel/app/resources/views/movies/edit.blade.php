...
@section('content')
    ...
    <form method="POST" action="/filmes/modificar/{{$movie->id}}" enctype="multipart/form-data">
        ...
        {{-- <div class="form-group col-md-6 col-sm-12">
            <img class="w-100" src="{{ $movie->cover_image }}" alt="">
        </div> --}}
        <div class="form-group col-md-6 col-sm-12">
            <label for="cover_image">Imagem</label>
            <input type="text" name="cover_image" value="{{ $movie->cover_image }}" class="form-control {{$errors->has('cover_image') ? ' is-invalid':''}}" id="cover_image">
